...
- QR: https://ibb.co/xxx
- Shopeepay: 08123456789 A/n Nama
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.warning("Gagal kirim logo.")
    ...
    await context.bot.send_message(LOGS_CHAT_ID, f"📥 User start/join:\nID: {uid}\nUsername: @{update.effective_user.username}\nWIB: {now_wib()}")
...
async def addkategori(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    await update.message.reply_text(f"✅ Kategori '{nama}' berhasil ditambahkan.")
...
async def delkategori(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        ...
        return
    if not context.args:
        await update.message.reply_text("/delkategori IDKategori")
        return
    try:
        cat_id = int(context.args[0])
        cursor.execute("DELETE FROM categories WHERE id=?", (cat_id,))
        conn.commit()
        await update.message.reply_text(f"❌ Kategori ID {cat_id} berhasil dihapus.")
    except:
        await update.message.reply_text("ID kategori harus angka.")
...
async def addproduk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        ...
        return
    try:
        # Split hanya sekali, ambil sisa text
        data = update.message.text.partition(" ")[2]
        parts = data.split("|")
        if len(parts) != 5:
            raise ValueError("Kurang argumen")
        nama, harga, stok, kategori_id, deskripsi = [p.strip() for p in parts]
        harga = int(harga.replace(".", "").replace(",", ""))
        stok = int(stok)
        kategori_id = int(kategori_id)
        cursor.execute(
            "INSERT INTO products (nama, harga, stok, kategori_id, deskripsi, file) VALUES (?, ?, ?, ?, ?, ?)",
            (nama, harga, stok, kategori_id, deskripsi, encrypt(""))
        )
        conn.commit()
        await update.message.reply_text(f"✅ Produk '{nama}' berhasil ditambahkan.")
        await context.bot.send_message(LOGS_CHAT_ID, f"➕ Produk baru:\nNama: {nama}\nHarga: {harga}\nStok: {stok}\nKategori ID: {kategori_id}")
    except Exception as e:
        logging.error(f"Error addproduk: {e}")
...
async def delproduk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_owner(update.effective_user.id):
        ...
        return
    if not context.args:
        await update.message.reply_text("/delproduk IDProduk")
        return
    try:
        prod_id = int(context.args[0])
        cursor.execute("DELETE FROM products WHERE id=?", (prod_id,))
        conn.commit()
        await update.message.reply_text(f"❌ Produk ID {prod_id} berhasil dihapus.")
    except:
...
async def listproduk_owner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cursor.execute("""SELECT p.id, p.nama, p.harga, p.stok, c.nama, p.deskripsi
    ...
    text = "📦 Daftar Produk:\n"
    for p in prods:
        text += f"ID:{p[0]} | {p[1]} | Rp{p[2]} | Stok:{p[3]} | Kategori:{p[4]}\nDeskripsi:{p[5]}\n\n"
    await update.message.reply_text(text)
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    await query.edit_message_text("Silakan transfer & kirim bukti.\nStatus: PENDING")
    cursor.execute("SELECT nama FROM products WHERE id=?", (pid,))
    prod_name = cursor.fetchone()[0]
    await context.bot.send_message(LOGS_CHAT_ID, f"🛒 Order baru:\nUser: {uid}\nProduk: {prod_name}\nStatus: PENDING\n{now_wib()}")
...
async def bukti(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with zipfile.ZipFile(mem_zip, 'w') as zf:
        for i, photo in enumerate(update.message.photo):
            ...
            bio.seek(0)
            zf.writestr(f"bukti_{i+1}.jpg", bio.read())
    mem_zip.seek(0)

    keyboard = [[InlineKeyboardButton("✅ ACC", callback_data=f"acc_{trx[0]}"),
                 InlineKeyboardButton("❌ CANCEL", callback_data=f"cancel_{trx[0]}")]]
    admins = OWNER_IDS.copy()
    cursor.execute("SELECT user_id FROM admins")
    for a in cursor.fetchall():
        admins.append(a[0])
    for admin_id in set(admins):
        try:
            await context.bot.send_document(admin_id, InputFile(mem_zip, filename=f"bukti_{trx[0]}.zip"),
                                            caption=f"User: {uid}\nTRX ID: {trx[0]}\n{now_wib()}",
                                            reply_markup=InlineKeyboardMarkup(keyboard))
        except:
            pass
    await update.message.reply_text("Bukti dikirim ke admin/owner.")
    await context.bot.send_document(LOGS_CHAT_ID, InputFile(mem_zip, filename=f"bukti_{trx[0]}.zip"),
                                    caption=f"📂 Bukti transaksi:\nUser ID: {uid}\nTRX ID: {trx[0]}\n{now_wib()}")
...
async def acc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(query.from_user.id):
        await query.answer("Bukan admin/owner!", show_alert=True)
        return
    ...
    decrypted = decrypt(file_enc)
    remaining = "\n".join(decrypted.split("\n")[1:]) if decrypted else ""
    cursor.execute("UPDATE products SET file=?, stok=stok-1 WHERE id=?", (encrypt(remaining), pid))
    ...
    await context.bot.send_message(LOGS_CHAT_ID, f"✅ Order ACC:\nUser: {uid}\nProduk: {prod_name}\nTRX ID: {trx_id}\n{now_wib()}")
    await query.edit_message_caption("✅ Transaksi ACC")
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.commit()
    await context.bot.send_message(uid, f"❌ Transaksi dibatalkan. TRX ID: {trx_id}")
    await context.bot.send_message(LOGS_CHAT_ID, f"❌ Order CANCEL:\nUser: {uid}\nTRX ID: {trx_id}\n{now_wib()}")
    await query.edit_message_caption("❌ Transaksi CANCEL")
...
app.add_handler(CallbackQueryHandler(list_produk, pattern="help"))
